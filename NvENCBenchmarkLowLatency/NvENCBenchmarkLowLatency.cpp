...
#include <string>
#include "NvENCBenchmarkLowLatency.h"
#include "nvUtils.h"
#include "nvFileIO.h"
...
NVENCSTATUS loadframe(uint8_t *yuvInput[3], HANDLE hInputYUVFile, uint32_t frmIdx, uint32_t width, uint32_t height, uint32_t &numBytesRead)
{
    ...
    result = nvSetFilePointer64(hInputYUVFile, 0, NULL, FILE_BEGIN);
...
int CNvEncoderLowLatency::EncodeMain(int argc, char *argv[])
{
    ...
    ZeroMemory(buf, MAX_PATH * 2 * sizeof(char));
    GetCurrentDirectoryA(MAX_PATH * 2, buf);
    sprintf_s(buf, MAX_PATH * 2, "%s\\NVENC_Encode_lowlatency_%04d%02d%02d_%02d%02d%02d.h264", buf, curTime.wYear, curTime.wMonth, curTime.wDay, curTime.wHour, curTime.wMinute, curTime.wSecond);
    ...
    for (int frm = 0; frm <= frameCount; frm++)
    {
        ...
        if (frm % 2)
        {
            loadframe(m_yuv, pfEven, frm, encodeConfig.width, encodeConfig.height, numBytesRead);
        }
        else
        {
            loadframe(m_yuv, pfOdd, frm, encodeConfig.width, encodeConfig.height, numBytesRead);
        }
        if (numBytesRead == 0)
            break;
